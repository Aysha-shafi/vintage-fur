        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          {serviceCards.map((card) => (
            <div key={card.title} className="p-4 flex flex-col items-center">
              <div className="mb-4 ">
                <div className="mb-4">
                  <img src={card.img} alt={card.title} className="w-20 h-20 object-contain" />
                </div>
              </div>
              ...
              <a href={card.link} className="text-lg  bg-yellow-500 rounded text-vintage-dark text-white py-3 px-6 hover:bg-black  transition duration-300">
                READ MORE
              </a>
            </div>
          ))}
